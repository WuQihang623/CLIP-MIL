...
class AlexNet_CIFAR10_attention(nn.Module):
    def __init__(self, features, num_classes, init=True, withoutAtten=False, input_feat_dim=192*3*3):
        ...
        if len(num_classes) == 1:
            self.top_layer = nn.Linear(512, num_classes[0])
        else:
...
class PromptLearner(nn.Module):
    def __init__(self, n_ctx=0, ctx_init=[], all_ctx_trainable=True, csc=True, classnames=["Negative", "Positive"], clip_model='ViT-B/32', p_drop_out=0.0):
...
class TOP(nn.Module):
    def __init__(self, prompt_learner_bagLevel, prompt_learner_instanceLevel, clip_model="ViT-B/32", pooling_strategy='NoCoOp'):
        super().__init__()
        ...
        self.pooling = AlexNet_CIFAR10_attention(features=None, num_classes=[2], input_feat_dim=512)
    ...
    def forward(self, image_raw):
        ...
        image = image_raw / image_raw.norm(dim=-1, keepdim=True)
        ...
        if self.pooling_strategy == 'ABMIL':
            ...
            output_dict = {}
            output_dict["features"] = image_features.mean(0)
            output_dict["logits"] = logits
            output_dict["attn"] = attn_score
            return output_dict
...
if __name__ == '__main__':
    ...
    bag_prompt_learner = PromptLearner(n_ctx=bagLevel_n_ctx,
                                       ctx_init=bagPrompt_ctx_init,
                                       all_ctx_trainable=all_ctx_trainable,
                                       csc=csc,
                                       classnames=["HER2 0", "HER2 1+", "HER2 2+", "HER2 3+"],
                                       clip_model='ViT-B/32', p_drop_out=p_bag_drop_out)
    ...
    prompts_pathology_template_withDescription = [
        pathology_templates_t.format(tissue_type).replace(".", ", which is {}".format(tissue_description)) for
        tissue_type, tissue_description in knowledge_from_chatGPT.items()]
    instancePrompt_ctx_init = [i + '* * * * * * * * * *' for i in prompts_pathology_template_withDescription]
    instance_prompt_learner = PromptLearner(n_ctx=instanceLevel_n_ctx,
                                            ctx_init=instancePrompt_ctx_init,
                                            all_ctx_trainable=all_ctx_trainable,
                                            csc=csc,
                                            classnames=["Prototype {}".format(i) for i in range(len(instancePrompt_ctx_init))],
                                            clip_model='ViT-B/32', p_drop_out=p_drop_out)

    model = TOP(bag_prompt_learner, instance_prompt_learner, clip_model="ViT-B/32", pooling_strategy=pooling_strategy)

    x = torch.randn((1024, 512))
    output = model(x)
    logits = output["logits"]
    attn_score = output["attn"]

    pred = logits.mean(0, keepdim=True).detach()
    print(pred.shape)
