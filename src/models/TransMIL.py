...
class TransMIL(nn.Module):
    def __init__(self, n_classes, feature_dim=1024):
        super(TransMIL, self).__init__()
        self.pos_layer = PPEG(dim=512)
        self._fc1 = nn.Sequential(nn.Linear(feature_dim, 512), nn.ReLU())
    ...
    def forward(self, h, return_attn=False):
        ...
        # ---->Translayer x1
        if return_attn:
            h, attn1, padding = self.layer1(h, return_attn)  # [B, N, 512]
            # print(h.shape)
            # print(attn1.shape)
            attn1 = attn1[:, :, 0, 1: (H+1)]
            # print(attn1.shape)
        else:
            h = self.layer1(h, return_attn)  # [B, N, 512]
        ...
        # ---->Translayer x2
        if return_attn:
            h, attn2, padding = self.layer2(h, return_attn)  # [B, N, 512]
            # print(attn2.shape)
            attn2 = attn2[:, :, 0, 1: (H+1)]
            # print(attn2.shape)
        else:
...
if __name__ == "__main__":
    data = torch.randn((1, 500, 1024))
    model = TransMIL(n_classes=3)
    model.eval()
    results_dict = model(data, True)
